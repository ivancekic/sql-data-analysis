--select PRO, TIP_D, DATUM_DOK, VRD, BRD, ULAZ, IZLAZ, STANJE, KOLICINA, CENA, CENA1, ODGOV, DATUM_UNOSA
--From
--(
...
-----------
       ,(case when nvl(lag (sd.proizvod,1) over (ORDER BY sd.proizvod, Datum_Unosa, Datum_dok),'xxxxxxx') <> proizvod then
              ...
              nvl(lag(sd.kolicina*sd.faktor*sd.k_robe,1) over (ORDER BY To_Number( Proizvod ) , Datum_Unosa, Datum_dok),0)
         end
        ) preth_stanje
-----------
...
--)
Order By  To_Number( Proizvod ), Datum_Unosa, Datum_dok
