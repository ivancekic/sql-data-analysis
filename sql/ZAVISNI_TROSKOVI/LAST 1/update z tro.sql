...
-- Datum: 03.06.2008
        ...
        nStatusDokumenta number;
        vVrstaMagacina varchar2(10);
        vVrsta_dok varchar2(3) := '3';
        vBroj_dok varchar2(9) := '12528';
        nGodina number := 2012;
           ...
           if suma_troskova_1 <> 0  or vrsta_Magacina = 'VP4'  then
              ...
              for x in (  select st.*
                        ...
                        if broj_stavki <> i then
                           z_troskovi_za_stavku := round(x.kolicina*round(x.cena *(100 -  x.rabat) / 100, 4)/uk_suma_dok * suma_troskova_1,2);

                           z_troskovi_za_stavku := round((round(
                                                               (x.kolicina * round(x.cena *(100 -  x.rabat) / 100, 4) + z_troskovi_za_stavku) / x.kolicina
                                                               ,4)
                                                          -
                                                          round(x.cena *(100 -  x.rabat) / 100, 4)) * x.kolicina
                                                   ,2);
                        else
...
--                                  UPDATE STAVKA_DOK
                             ...
                             WHEN vrsta_Magacina = 'VP4' THEN
                                  If nStatusDokumenta = -8 then
                                            nCena1 := round((  (x.kolicina * x.cena * (1-x.rabat/100) + nvl(z_troskovi_za_stavku,0)) / x.kolicina)
                                                             *(100 + nPreprodajniProcenat ) / 100,2);
                                  End if;
                             --------------------deja
                             WHEN vrsta_Magacina = 'NAB'    THEN
                                            nCena1 := round(
                                                             (x.kolicina * round(x.cena *(100 -  x.rabat) / 100, 4) + z_troskovi_za_stavku)
                                                              / x.kolicina
                                                          ,4);
                             ...
                             ELSE
                                  NULL;
                        END CASE;
Dbms_output.put_line('cena ' || to_char(nCena1));
