...
--               TRUNC((MOD(ROWNUM-1,BrRedNaStranici)-ROWNUM)*(-1)/BrRedNaStranici,0)+1 STRANICA,
        ...
        select
               ...
               sum(NVL(round(nvl(sign(ST.K_ROBE)*ST.KOLICINA*ST.FAKTOR*ST.K_ROBE*st.cena1
                   *PDokument.ZokiUlazIzlaz(d.vrsta_dok,d.tip_dok,st.K_ROBE,'D',d.datum_dok),0),2),0)*
                   ...
                   case when d.vrsta_dok in (80) then
                        ...
                        WHEN d.vrsta_dok in (90) then
                             ...
                             *
                             ...
                             ROUND(st.kolicina*(NVL(st.cena,0)-NVL(st.cena1,0)),2)
                        else
                           0
                   end
                   )
               ...
               rabat,
               sum(NVL(round(nvl(sign(ST.K_ROBE)*ST.KOLICINA*ST.FAKTOR*DECODE(ST.VRSTA_DOK,'90',0,ST.K_ROBE)*st.cena ----(1-ST.RABAT/100)
                   *PDokument.ZokiUlazIzlaz(d.vrsta_dok,d.tip_dok,st.K_ROBE,'P',d.datum_dok),0),2),0))
               razduzenje

          from dokument d,(SELECT  BROJ_DOK,
                                   VRSTA_DOK,
                                   GODINA,
